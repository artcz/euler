...
def solve():
    """
    TODO: possibly refactor, and make a nicer version.
    """
    adjacent_numbers = 4
    HORIZONTAL_PRODUCTS = []
    VERTICAL_PRODUCTS = []
    ...
    def product(*n):
        "Gets strings as arguments, returns product of all of them"
        return reduce(lambda x, y: x*y, (int(p) for p in n))
    ...
    def naive():
        ...
        for line in GRID:
            for n in range(len(line)-adjacent_numbers+1):
                numbers = line[n:n+adjacent_numbers]
                HORIZONTAL_PRODUCTS.append(product(*numbers))
        ...
        for col_index in range(max_col):
            for row_index in range(max_row-adjacent_numbers+1):
                columnar = []
                for i in range(adjacent_numbers):
                    columnar.append(GRID[row_index+i][col_index])

                VERTICAL_PRODUCTS.append(product(*columnar))
        ...
        for col_index in range(max_col-adjacent_numbers):
            for row_index in range(max_row-adjacent_numbers+1):
                columnar = []
                for i in range(adjacent_numbers):
                    columnar.append(GRID[row_index+i][col_index+i])

                DIAGONAL_PRODUCTS.append(product(*columnar))
        ...
        print "Horizontal:  ", max(HORIZONTAL_PRODUCTS)
        print "Vertical:  ", max(VERTICAL_PRODUCTS)
        print "Diagonal:  ", max(DIAGONAL_PRODUCTS)
